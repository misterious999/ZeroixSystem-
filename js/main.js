...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (session.role === 'user' && session.expiredAt > 0 && Date.now() > session.expiredAt) {
      ...
      setTimeout(() => { localStorage.removeItem('session'); window.location.href = 'login.html'; }, 3000);
      return; 
  }
  ...
  if (path === 'dashboard.html' || path === '') {
    if (session.role === 'admin') {
      ...
      contentDiv.innerHTML = `
        ...
        <div class="glass-card"><h3 style="margin-bottom: 15px;">Aksi Cepat</h3><div style="display:flex; gap:15px;"><a href="admin-users.html" class="btn btn-primary">Kelola User</a></div></div>
      `;
    } else {
      ...
      contentDiv.innerHTML = `
        ...
        <div class="glass-card">
           ...
           <div style="margin-bottom:15px;"><div style="display:flex; justify-content:space-between; margin-bottom:5px;"><label>RAM Usage</label><span id="stat-ram">0 MB</span></div><div style="width:100%; background:rgba(0,0,0,0.3); border-radius:10px; height:12px; overflow:hidden;"><div id="bar-ram" style="width:0%; height:100%; background:var(--accent-cyan); transition:0.5s;"></div></div></div>
           <div style="margin-bottom:15px;"><div style="display:flex; justify-content:space-between; margin-bottom:5px;"><label>CPU Usage</label><span id="stat-cpu">0%</span></div><div style="width:100%; background:rgba(0,0,0,0.3); border-radius:10px; height:12px; overflow:hidden;"><div id="bar-cpu" style="width:0%; height:100%; background:var(--accent-gold); transition:0.5s;"></div></div></div>
           <div style="margin-bottom:15px;"><div style="display:flex; justify-content:space-between; margin-bottom:5px;"><label>Disk Storage</label><span id="stat-disk">0 MB</span></div><div style="width:100%; background:rgba(0,0,0,0.3); border-radius:10px; height:12px; overflow:hidden;"><div id="bar-disk" style="width:0%; height:100%; background:var(--accent-violet); transition:0.5s;"></div></div></div>
        </div>
      `;
          ...
          try {
              ...
              if(srvSnap.exists()) {
                  ...
                  if(myServer && myServer.pteroIdentifier) {
                      const statRes = await fetch(`${pteroApiUrl}/api/client/servers/${myServer.pteroIdentifier}/resources`, { headers: { "Accept": "application/json", "Authorization": `Bearer ${window.CONFIG.ptero.clientApiKey || pteroConfig.apiKey}` } });
                      if(statRes.ok) {
                          ...
                          document.getElementById('stat-ram').innerText = `${ramUsed} MB`; document.getElementById('bar-ram').style.width = `${Math.min((ramUsed/1024)*100, 100)}%`;
                          document.getElementById('stat-cpu').innerText = `${cpuUsed}%`; document.getElementById('bar-cpu').style.width = `${Math.min(cpuUsed, 100)}%`;
                          document.getElementById('stat-disk').innerText = `${diskUsed} MB`; document.getElementById('bar-disk').style.width = `${Math.min((diskUsed/5120)*100, 100)}%`;
                      }
                  }
              }
          } catch(e) {}
  ...
  // =====================================================================
  // 2. HALAMAN ADMIN-USERS (KELOLA PENGGUNA)
  // =====================================================================
    ...
    if(snap.exists()) {
      ...
      Object.entries(snap.val()).forEach(([uname, data]) => {
        ...
        tbody.innerHTML += `<tr><td><b>${window.utils.escapeHtml(uname)}</b></td><td>${window.utils.escapeHtml(data.name)}</td><td><a href="https://wa.me/${data.whatsapp}" target="_blank" class="text-cyan">${data.whatsapp}</a></td><td>${teksSisaWaktu}</td><td>${badge}</td><td><button class="btn-icon" onclick="perpanjangUser('${uname}', ${data.expiredAt || 0})"><i data-lucide="calendar-clock" class="text-gold"></i></button><button class="btn-icon" onclick="hapusUser('${uname}')"><i data-lucide="trash-2" class="text-danger"></i></button></td></tr>`;
      });
    }
    ...
    window.submitAddUser = async (e) => {
        ...
        btn.innerHTML = '<i data-lucide="loader" class="spin"></i> Menghubungi Panel...';
        btn.disabled = true;

        const uname = document.getElementById('addUname').value.toLowerCase().replace(/[^a-z0-9_]/g, '');
        ...
        try {
            ...
            if (!pteroUserRes.ok) {
                ...
                throw new Error(`Gagal User: ${err.errors?.[0]?.detail || "Periksa API Key Panel"}`);
            }
            ...
            console.error("API Panel Error:", error);
            ...
            if(srvSnap.exists()) {
                for(let [sId, sData] of Object.entries(srvSnap.val())) {
                    if(sData.ownerUsername === username) {
                        if (sData.pteroServerId) fetch(`${pteroApiUrl}/api/application/servers/${sData.pteroServerId}/force`, { method: 'DELETE', headers: pteroHeaders });
                        if (sData.pteroUserId) fetch(`${pteroApiUrl}/api/application/users/${sData.pteroUserId}`, { method: 'DELETE', headers: pteroHeaders });
                        await remove(ref(db, `servers/${sId}`));
                    }
                }
            }
            await remove(ref(db, `users/${username}`));
            window.utils.showToast('User dihapus!', 'success');
            setTimeout(() => location.reload(), 1000);
  ...
  // =====================================================================
    ...
    contentDiv.innerHTML = `
      <div class="glass-card" style="margin-bottom:25px;"><h3>${isUserMode ? 'Daftar Server Kamu' : 'Daftar Seluruh Server'}</h3><p class="text-muted" style="font-size:0.9rem;">Terkoneksi langsung ke database.</p></div>
      <div class="glass-card" style="overflow-x:auto;">
        <table id="servers-table"><thead><tr><th>ID Server</th><th>Pemilik</th><th>Nama Server</th><th>Status</th>${isUserMode ? '' : '<th>Aksi</th>'}</tr></thead><tbody><tr><td colspan="${isUserMode ? '4' : '5'}" style="text-align:center;"><i data-lucide="loader" class="spin"></i> Memuat...</td></tr></tbody></table>
      </div>
    `;
    ...
    if(srvSnap.exists() && Object.keys(srvSnap.val()).length > 0 && !srvSnap.val()._placeholder) {
      ...
      Object.entries(srvSnap.val()).forEach(([sId, sData]) => {
        if(sId === '_placeholder' || (isUserMode && sData.ownerUsername !== session.username)) return;
        const badgeStatus = sData.status === 'active' ? '<span class="badge-cyan">Active</span>' : '<span class="badge-danger">Suspended</span>';
        tbody.innerHTML += `<tr><td><code>${sId.substring(0,8)}</code></td><td><b>${sData.ownerUsername || 'Unknown'}</b></td><td>${window.utils.escapeHtml(sData.name || 'Server')}</td><td>${badgeStatus}</td>${isUserMode ? '' : `<td><button class="btn-icon" onclick="hapusServer('${sId}')" title="Hapus Server"><i data-lucide="trash-2" class="text-danger"></i></button></td>`}</tr>`;
      });
      if(tbody.innerHTML === '') tbody.innerHTML = `<tr><td colspan="${isUserMode ? '4' : '5'}" style="text-align:center;">Tidak ada server.</td></tr>`;
    } else {
      tbody.innerHTML = `<tr><td colspan="${isUserMode ? '4' : '5'}" style="text-align:center;">Belum ada server.</td></tr>`;
    }

    if(!isUserMode) {
      window.hapusServer = async (serverId) => {
          if(!confirm(`Yakin hapus Server ${serverId}?`)) return;
          try {
              const srvSnap = await get(ref(db, `servers/${serverId}`));
              if(srvSnap.exists() && srvSnap.val().pteroServerId) {
                  await fetch(`${pteroApiUrl}/api/application/servers/${srvSnap.val().pteroServerId}/force`, { method: 'DELETE', headers: pteroHeaders });
              }
              await remove(ref(db, `servers/${serverId}`));
              window.utils.showToast(`Server dihapus!`, 'success');
